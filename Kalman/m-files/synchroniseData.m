% Synchronize the data of the experiments (KUKA; AZUREKINECT; VISIONRTK)  
%   synchroniseData.m 
%
...
function [Kuka_KF, AzureKinect_KF, VisionRTK_KF] = synchroniseData(Kuka, AzureKinect, VisionRTK, timeshift)
...
Kuka_KF.trajectory.position = Kuka.trajectory.position- median(Kuka.trajectory.position(1:50,:));
Kuka_KF.trajectory.orientation = Kuka.trajectory.orientation - median(Kuka.trajectory.orientation(1:50,:));
...
timeImu_ = AzureKinect.Imu.timestamp + timeshift - Kuka.timestamp(1,1);
...
AzureKinect_KF.Imu.timestamp = timeImu_(startGnssImu:end);
AzureKinect_KF.Imu.specificforce = AzureKinect.Imu.specificforce(startGnssImu :end,:);
AzureKinect_KF.Imu.angularrate = AzureKinect.Imu.angularrate(startGnssImu:end,:);
